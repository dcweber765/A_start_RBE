...
def get_neighbors(loc, my_map):
    """
        returns the legal AND OPEN neighbors of loc
    ...
    for i in range(-1,2):
        for j in range(-1,2):
            if is_open_loc((x+i, y+j), my_map): #and i != 0 and j != 0:
                neighbors.append((x+i, y+j))  #open cells above and below and left and right of current cell

    return neighbors

def get_all_neighbors(loc, my_map):
    """
        returns the legal (INBOUND) neighbors of loc
        :param loc: tuple of location
        :return: list of tuples
    """
    x = loc[0]; y = loc[1]
    ...
    for i in range(-1,2):
        for j in range(-1,2):
            ...
            if is_valid_loc((x+i, y+j), my_map):
                neighbors.append((x+i, y+j))  #open cells above and below and left and right of current cell
...
def is_open_loc(loc, my_map):
    ...
    if is_valid_loc(loc, my_map):
        return my_map.data[point_to_index(loc, my_map)] < 50 and my_map.data[point_to_index(loc, my_map)] >= 0#checks to see if Occupancy of cell is empty
    else:
        return False
...
def enlarge_obstacles(my_map):
    ...
    #                    newData[newIndex] = 100
    #         elif my_map.data[index] == -1:
    #             newData[index] = -1

    for i in range(0, width):
        for j in range(0, height):
            index = point_to_index((i, j), my_map)
            if my_map.data[index] > 50:
                newIndex = point_to_index((i, j), my_map)
                newData[newIndex] = 100
                for k in range(-1*buf, buf + 1):
                    for l in range(-1*buf, buf + 1):
                        x = i + k
                        y = j + l
                        if is_valid_loc((x, y), my_map):
                           newIndex = point_to_index((x, y), my_map)
...
def point_to_world(tuple, my_map):
    ...
    worldX = (tuple[0] * resolution) + (resolution/2) #+ xOrigin
    worldY = (tuple[1] * resolution) + (resolution/2) #+ yOrigin
...
def increaseResolution(my_map):

    newMap = OccupancyGrid()


    
    newMap.info.width = my_map.info.width*2
    newMap.info.height = my_map.info.height*2

    newMap.info.resolution = my_map.info.resolution/2

    newData = []
    for i in range(0, newMap.info.width):
        for j in range(0, newMap.info.height):
            newData.append(0)

    for i in range(0, my_map.info.width):
        for j in range(0, my_map.info.height):
            for k in range(0, 2):
                for l in range(0, 2):
                    index = point_to_index((((i*2)+k),(((j*2)+l))), my_map)
                    #print index
                    newData[index] = 0#my_map.data[point_to_index((i,j), my_map)]
